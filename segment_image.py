...
def load_image_pixels(filename, shape):
    ...
    image = keras.preprocessing.image.load_img(filename, target_size=shape)
...
def get_boxes(boxes, labels, thresh):
    ...
    for box in boxes:
        ...
        for i in range(len(labels)):
            ...
            class_Th.append(box.classes[i])
            if box.classes[i] > thresh:
                v_boxes.append(box)
                v_labels.append(labels[i])
                v_scores.append(box.classes[i] * 100)
            # don't break, many labels may trigger for one box
    return v_boxes, v_labels, v_scores,class_Th


# draw all results
def draw_boxes(filename, v_boxes, v_labels, v_scores, path):
    # load the image
...
def segment(adder, newName, user_id):
    pyplot.clf()
    pyplot.cla()
    pyplot.close()
    ...
    # load yolov3 model
    model = tf.keras.models.load_model('static/models/segment/m1-2.h5')
    model.summary()
    ...
    v_boxes, v_labels, v_scores, class_Th = get_boxes(boxes, labels, class_threshold)
    ...
    draw_boxes(photo_filename, v_boxes, v_labels, v_scores, path1)
    boxes = list()
    for i in range(len(yhat)):
        # decode the output of the network
        boxes += decode_netout(yhat[i][0], anchors[i], class_threshold, input_h, input_w)
